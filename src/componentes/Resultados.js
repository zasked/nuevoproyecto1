...
//                                                      
  ...
  //  https://api.themoviedb.org/3/search/movie?api_key=8f2a16d4f0d2593718febacdf7b1d495&language=es-ES&query=spider"
            ...
            movieResults.map((pelis, i) => {
                return ( 
                    ...
                    <div className="card my-3 img-fluid"   style={{ width: '15rem' }}  >
                        <img className="card-img-top img-fluid" src={`https://image.tmdb.org/t/p/w500/${pelis.poster_path}`} alt="Card"/>
                        <button type="buttom" className="favourite-btn"/>
                        <div className="card-body">
                            <h5 className="card-title" key={i}>{pelis.title?pelis.title.substring(0,50):null}</h5>

                            <Link to={`/detalle?pelisID=${pelis.id}`} className="btn btn-primary">Go somewhere</Link>
                        </div>
                    </div>
                    </div>
                )
